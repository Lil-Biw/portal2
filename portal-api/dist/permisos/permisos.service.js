"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PermisosService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
let PermisosService = class PermisosService {
    constructor(permisoModel) {
        this.permisoModel = permisoModel;
    }
    async asignar(dto, asignadoPor, clienteId) {
        const existe = await this.permisoModel.findOne({
            usuario_id: dto.usuario_id,
            centro_costo_id: dto.centro_costo_id,
        });
        if (existe) {
            return this.permisoModel.findByIdAndUpdate(existe._id, { tipo: dto.tipo }, { new: true }).lean();
        }
        const doc = { ...dto };
        if (clienteId)
            doc.cliente_id = new mongoose_2.Types.ObjectId(clienteId);
        if (asignadoPor)
            doc.asignado_por = new mongoose_2.Types.ObjectId(asignadoPor);
        return new this.permisoModel(doc).save();
    }
    async findByUsuario(usuario_id) {
        return this.permisoModel
            .find({ usuario_id: new mongoose_2.Types.ObjectId(usuario_id) })
            .populate('centro_costo_id', 'codigo nombre')
            .lean();
    }
    async findByCentro(centro_costo_id) {
        return this.permisoModel
            .find({ centro_costo_id: new mongoose_2.Types.ObjectId(centro_costo_id) })
            .populate('usuario_id', 'nombre email')
            .lean();
    }
    async revocar(usuario_id, centro_costo_id) {
        const resultado = await this.permisoModel.deleteOne({
            usuario_id: new mongoose_2.Types.ObjectId(usuario_id),
            centro_costo_id: new mongoose_2.Types.ObjectId(centro_costo_id),
        });
        if (resultado.deletedCount === 0) {
            throw new common_1.NotFoundException('No se encontró el permiso para revocar');
        }
        return { message: 'Permiso revocado correctamente' };
    }
};
exports.PermisosService = PermisosService;
exports.PermisosService = PermisosService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)('Permiso')),
    __metadata("design:paramtypes", [mongoose_2.Model])
], PermisosService);
//# sourceMappingURL=permisos.service.js.map